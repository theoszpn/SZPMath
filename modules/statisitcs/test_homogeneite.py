...
class ChiSquareHomoPage(QWidget):
    def __init__(self):
        super().__init__()
        self.cell_size = 65
        self.init_ui()
    ...
    def init_ui(self):
        ...
        """
        self.btn_run = QPushButton("➜ Test d'homogénéité du χ²")
    ...
    def run_chi2_test(self):
        try:
            ...
            self.log_area.clear()
            self.log_area.insertHtml("<h2>Test d'homogénéité du Khi-deux</h2>")

            self.log_area.append(
                f"On cherche ici à mesurer l'homogénéité de la répartition d'un caractère X parmis des groupes Y au seuil de confiance <b>α = {alpha}</b>. Pour représenter nos données on utilise un tableau de contingence.<br><br>")
            ...
            self.log_area.insertHtml(
                "<br>On pose les hypothèses suivantes :<br><b>H₀</b> : Les populations sont homogènes (la répartition du caractère est la même pour tous les groupes).<br><b>H₁</b> : Les populations ne sont pas homogènes (il existe une différence de répartition).<br><br>")
            self.log_area.append(
                "Pour évaluer ces hypothèses, on compare les <b>effectifs observés nᵢⱼ</b> aux <b>effectifs théoriques eᵢⱼ</b> correspondants à l'hypothèse d'homogénéité.")
